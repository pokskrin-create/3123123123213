...
class NumbersKt__FloorDivModKt extends NumbersKt__BigIntegersKt {
    ...
    private static final double mod(float f, double d) {
        double d2 = f % d;
        return (d2 == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || Math.signum(d2) == Math.signum(d)) ? d2 : d2 + d;
    }

    private static final double mod(double d, float f) {
        double d2 = f;
        double d3 = d % d2;
        return (d3 == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || Math.signum(d3) == Math.signum(d2)) ? d3 : d3 + d2;
    }
