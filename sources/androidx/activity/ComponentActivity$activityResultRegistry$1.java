...
public final class ComponentActivity$activityResultRegistry$1 extends ActivityResultRegistry {
    ...
    public <I, O> void onLaunch(final int requestCode, ActivityResultContract<I, O> contract, I input, ActivityOptionsCompat options) {
        ...
        final ActivityResultContract.SynchronousResult<O> synchronousResult = contract.getSynchronousResult(componentActivity3, input);
        ...
        Intent intentCreateIntent = contract.createIntent(componentActivity3, input);
        if (intentCreateIntent.getExtras() != null) {
            Bundle extras = intentCreateIntent.getExtras();
            Intrinsics.checkNotNull(extras);
            if (extras.getClassLoader() == null) {
                intentCreateIntent.setExtrasClassLoader(componentActivity2.getClassLoader());
            }
        }
        ...
        if (Intrinsics.areEqual(ActivityResultContracts.RequestMultiplePermissions.ACTION_REQUEST_PERMISSIONS, intentCreateIntent.getAction())) {
            ...
            ActivityCompat.requestPermissions(componentActivity2, stringArrayExtra, requestCode);
            return;
        }
        ...
        ActivityCompat.startActivityForResult(componentActivity2, intentCreateIntent, requestCode, bundle2);
    }
