...
function calendarSchduleAdd() {
	...
	$.ajax({
		url: '/hs/admin/googleCalendar/HsAdminGoogleCalendarEventAdd.json',
		type: 'post',
		async: false,
...
function screenWriteMonth() {
	...
	console.log("data.chk==>>", data.chk);
	if(data.chk) {
		for(var i=0; i<data.cnt; i++) {
			console.log("data.cnt==>>" + data.cnt + "==>>", i);
...
function modifyEvent() {
	...
	$.ajax({
		url: '/hs/admin/googleCalendar/HsAdminGoogleCalendarEventModify.json',
		type: 'post',
		async: false,
...
function removeEventOne(eventId) {
	$.ajax({
		url: '/hs/admin/googleCalendar/HsAdminGoogleCalendarEventRemoveOne.json',
		type: 'post',
		async: false,
		data : {
			"eventId" : eventId,
...
function calendarEventList() {
	$.ajax({
		url: '/hs/admin/googleCalendar/HsAdminGoogleCalendarEventList.json',
		type: 'post',
		data: {
			"calendarId" : $('#calendarId').val()
		},
		...
		success: function(dataAll) {
			...
			console.log("dataAll.items==>>", dataAll.items);
			if ( dataAll.items != undefined ) {
				var lists = dataAll.items;
				if(lists.length != 0) {
					data.chk = true;
