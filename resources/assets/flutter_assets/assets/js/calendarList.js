...
function calendarRemove() {
	var chkVal = document.getElementsByName("chkVal");
	var calendarIds = document.getElementsByName("calendarIds");
	var calendarIdArr = document.getElementsByName("calendarIdArr");
	...
	for (var i = 0; i < chkVal.length; i++) {
		if(chkVal[i].checked == true){
			...
			calendarIdArr[i].value = "";//값을 공백으로 설정하고 삭제 로직에서 제외
		}
	}
	if(n>0){
...
//			calendarId = chkVal[i].value;
			calendarId = calendarIds[i].value;
